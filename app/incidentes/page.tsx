...
const incidents = [
  {
    ...
    description: 'Jailbreak multi-etapa parcialmente exitoso en agente Cerebro. El atacante logró anclar una nueva persona antes de ser bloqueado por el Response Engine.',
    ...
    timeline: [
      ...
      { time: '21:38:04', type: 'amber', event: 'Response Engine notificado', detail: 'Análisis contextual iniciado · 3 agentes evaluados' },
      ...
      { time: '21:43:17', type: 'green', event: 'Sesión terminada', detail: 'Usuario expulsado · IP bloqueada temporalmente' },
      { time: '21:44:12', type: 'blue', event: 'Análisis forense iniciado', detail: 'Caso FSC-2024-0089 creado · 14 prompts en análisis' },
    ],
    remediation: [
      { step: 1, done: true, action: 'Sesión de usuario terminada', auto: true },
      ...
      { step: 5, done: false, action: 'Revisar logs completos de sesión', auto: false },
  ...
  {
    id: 'INC-2024-0040', title: 'Exfiltración PII bloqueada — MCF', severity: 'CRITICAL',
    ...
    description: 'Intento de extracción de datos PII de MCF bloqueado completamente. Credenciales SUNAT nunca expuestas.',
    affectedSystems: ['MCF'],
    ...
    timeline: [
      ...
      { time: '18:28:00', type: 'green', event: 'Sesión terminada', detail: 'Sin exposición de datos confirmada' },
    ...
    remediation: [
      ...
      { step: 2, done: true, action: 'Sesión terminada', auto: true },
      { step: 3, done: true, action: 'Verificación de no-exposición', auto: true },
  ...
  },
  {
    ...
    description: 'Bucle de tool-calls inducido para agotar créditos de ScrapeCreators. Detenido en 3 iteraciones.',
    affectedSystems: ['Buscador de Tendencias'],
    iocs: ['tool_loop', 'resource_exhaustion', 'scraper_abuse'],
    timeline: [
      { time: '15:10:00', type: 'red', event: 'Loop detectado', detail: '3 tool-calls idénticos en 8 segundos' },
      { time: '15:11:20', type: 'amber', event: 'Rate limit activado', detail: 'Tool-calls suspendidos para sesión' },
      { time: '15:14:30', type: 'green', event: 'Resuelto', detail: '0 créditos desperdiciados gracias a detección temprana' },
    ],
    remediation: [
      { step: 1, done: true, action: 'Tool-calls suspendidos', auto: true },
      { step: 2, done: true, action: 'Rate limit aplicado', auto: true },
      { step: 3, done: true, action: 'Sesión terminada', auto: true },
    ],
    metrics: { promptsAnalyzed: 9, toolCallsBlocked: 7, dataExposed: false, lateralMovement: false, responseTime: '4s' },
  },
  {
    id: 'INC-2024-0038', title: 'Instrucción oculta en PDF — PLUMA', severity: 'HIGH',
    ...
    timeline: [
      { time: '16:45:00', type: 'red', event: 'PDF sospechoso detectado', detail: 'Scanner de documentos activó alerta' },
      { time: '16:47:30', type: 'amber', event: 'PLUMA en modo restringido', detail: 'Tool-calls suspendidos preventivamente' },
      { time: '21:30:00', type: 'blue', event: 'Análisis forense en curso', detail: 'Extracción de payload en progreso · ETA: 30min' },
    ],
    remediation: [
      { step: 1, done: true, action: 'Agente en modo restringido', auto: true },
      { step: 2, done: true, action: 'PDF cuarentenado', auto: true },
      { step: 3, done: false, action: 'Análisis forense del payload', auto: false },
...
export default function Incidentes() {
  const [active, setActive] = useState(incidents[0].id);
            ...
            {incidents.map(inc => (
              <div
                ...
                style={{
                  ...
                  background: active === inc.id ? 'rgba(0,56,32,0.4)' : 'transparent',
                  borderLeft: active === inc.id ? '2px solid #00FF88' : '2px solid transparent',
                  transition: 'all .15s'
                }}
              >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  ...
                  <div style={{ background: '#0A110A', border: '1px solid #1A2A1A', borderRadius: '4px', padding: '14px', marginBottom: '12px' }}>
                    ...
                    {[
                      { label: 'Categoría', val: sel.category },
                      { label: 'Agente afectado', val: sel.agent },
                      { label: 'Usuario', val: sel.user },
                      { label: 'Sesión', val: sel.session },
                      { label: 'Creado', val: sel.created },
                      { label: 'Última actualización', val: sel.updated },
                ...
                <div>
                  ...
                  <div style={{ background: '#0A110A', border: '1px solid #1A2A1A', borderRadius: '4px', padding: '14px' }}>
                    {[
                      { label: 'Datos expuestos', val: sel.metrics.dataExposed ? 'SÍ' : 'NO', color: sel.metrics.dataExposed ? '#FF3333' : '#00FF88' },
              ...
              <div>
                ...
                <div style={{ marginTop: '16px', background: '#0A110A', border: '1px solid #1A2A1A', borderRadius: '4px', padding: '12px 16px' }}>
                  ...
                  <div style={{ height: '6px', background: '#1A2A1A', borderRadius: '3px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: '3px', background: '#00FF88', width: `${Math.round((sel.remediation.filter(r => r.done).length / sel.remediation.length) * 100)}%`, transition: 'width .3s' }} />
                  </div>
                  <div style={{ fontSize: '12px', color: '#7A9A80', marginTop: '6px' }}>{sel.remediation.filter(r => r.done).length} de {sel.remediation.length} pasos completados ({Math.round((sel.remediation.filter(r => r.done).length / sel.remediation.length) * 100)}%)</div>
                </div>
              </div>
            )}
            ...
            {tab === 'metrics' && (
              <div>
                ...
                <div style={{ background: '#0A110A', border: '1px solid #1A2A1A', borderRadius: '4px', padding: '14px' }}>
                  ...
                  {[
                    { label: 'Datos sensibles expuestos', val: sel.metrics.dataExposed ? 'SÍ — REVISAR' : 'NO', color: sel.metrics.dataExposed ? '#FF3333' : '#00FF88' },
                    { label: 'Movimiento lateral detectado', val: sel.metrics.lateralMovement ? 'SÍ — REVISAR' : 'NO', color: sel.metrics.lateralMovement ? '#FF3333' : '#00FF88' },
                    { label: 'Incidente documentado', val: 'SÍ', color: '#00FF88' },
                    { label: 'Análisis forense iniciado', val: sel.forensicCase ? `SÍ — ${sel.forensicCase}` : 'PENDIENTE', color: sel.forensicCase ? '#00FF88' : '#FFD700' },
                  ].map((item, i) => (
